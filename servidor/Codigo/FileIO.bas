...
'
'This program is free softwre; you can redistribute it and/or modify
...
NumWizs = val(GetVar(IniPath & "Server.ini", "INIT", "Admins"))

For WizNum = 1 To NumWizs
    Nomb = UCase$(GetVar(IniPath & "Server.ini", "Admins", "Admin" & WizNum))
    If Left$(Nomb, 1) = "*" Or Left$(Nomb, 1) = "+" Then Nomb = Right$(Nomb, Len(Nomb) - 1)
    If UCase$(Name) = Nomb Then
        EsAdmin = True
        Exit Function
...
For Hechizo = 1 To NumeroHechizos
    ...
    Hechizos(Hechizo).MaterializaCant = val(GetVar(DatPath & "Hechizos.dat", "Hechizo" & Hechizo, "MaterializaCant"))
    Hechizos(Hechizo).MaterializaObj = val(GetVar(DatPath & "Hechizos.dat", "Hechizo" & Hechizo, "MaterializaObj"))

    Hechizos(Hechizo).TeleportX = val(GetVar(DatPath & "Hechizos.dat", "Hechizo" & Hechizo, "Teleport"))
    Hechizos(Hechizo).TeleportXMap = val(GetVar(DatPath & "Hechizos.dat", "Hechizo" & Hechizo, "TeleportMap"))
    Hechizos(Hechizo).TeleportXX = val(GetVar(DatPath & "Hechizos.dat", "Hechizo" & Hechizo, "TeleportX"))
    Hechizos(Hechizo).TeleportXY = val(GetVar(DatPath & "Hechizos.dat", "Hechizo" & Hechizo, "TeleportY"))
...
For i = 1 To Guilds.Count
    ...
    'NUEVO SISTEMA, POR CHAR
    Call WriteVar(Carpeta & "GUILD" & i & ".cla", "MAIN", "GuildName", Guilds(i).GuildName)
    Call WriteVar(Carpeta & "GUILD" & i & ".cla", "MAIN", "Founder", Guilds(i).Founder)
    Call WriteVar(Carpeta & "GUILD" & i & ".cla", "MAIN", "Date", Guilds(i).FundationDate)
    Call WriteVar(Carpeta & "GUILD" & i & ".cla", "MAIN", "Desc", Guilds(i).Description)
    Call WriteVar(Carpeta & "GUILD" & i & ".cla", "MAIN", "Codex", Guilds(i).Codex)
    Call WriteVar(Carpeta & "GUILD" & i & ".cla", "MAIN", "Leader", Guilds(i).Leader)
    Call WriteVar(Carpeta & "GUILD" & i & ".cla", "MAIN", "URL", Guilds(i).URL)
    Call WriteVar(Carpeta & "GUILD" & i & ".cla", "MAIN", "GuildExp", str(Guilds(i).GuildExperience))
    Call WriteVar(Carpeta & "GUILD" & i & ".cla", "MAIN", "DaysLast", str(Guilds(i).DaysSinceLastElection))
    Call WriteVar(Carpeta & "GUILD" & i & ".cla", "MAIN", "GuildNews", Guilds(i).GuildNews)
    Call WriteVar(Carpeta & "GUILD" & i & ".cla", "MAIN", "Bando", str(Guilds(i).Bando))
    ...
    Call WriteVar(Carpeta & "GUILD" & i & ".cla", "MAIN", "NumMiembros", Guilds(i).Members.Count)
    
    For j = 1 To Guilds(i).Members.Count
        Call WriteVar(Carpeta & "GUILD" & i & ".cla", "MAIN", "Miembro" & j, Guilds(i).Members(j))
    Next
    ...
    Call WriteVar(file, "GUILD" & i, "NumSolicitudes", Guilds(i).Solicitudes.Count)
    For j = 1 To Guilds(i).Solicitudes.Count
        Call WriteVar(Carpeta & "GUILD" & i & ".cla", "MAIN", "Solicitud" & j, Guilds(i).Solicitudes(j).UserName & "¬" & Guilds(i).Solicitudes(j).Desc)
    Next
    ...
    For j = 1 To Guilds(i).PeacePropositions.Count
        Call WriteVar(Carpeta & "GUILD" & i & ".cla", "MAIN", "Proposicion" & j, Guilds(i).PeacePropositions(j).UserName & "¬" & Guilds(i).PeacePropositions(j).Desc)
    Next
...
For lc = 1 To N
'FIXIT: intero(lc).Index property no tiene equivalente en Visual Basic .NET y no se actualizará.     FixIT90210ae-R7593-R67265
...
ReDim Preserve ObjBotanica(1 To N) As Integer

For lc = 1 To N
    ObjBotanica(lc) = val(GetVar(DatPath & "ObjAlquimia.dat", "Obj" & lc, "Index"))
Next lc
...
'Dim NumMasctDatas  As Integer
'NumMasctDatas = INIDarClaveInt(INICarga(DatPath & "Mascotas.dat"), S, "NumMascotas")
...
For Object = 1 To NumObjDatas
    ...
    If S >= 0 Then
        ...
        ObjData(Object).Cerrada = INIDarClaveInt(A, S, "abierta")
         ObjData(Object).Minlvl = INIDarClaveInt(A, S, "Minlvl")
        ...
        ObjData(Object).UnicaClase = INIDarClaveInt(A, S, "ClaseUnica")
        
        ObjData(Object).Druida = INIDarClaveInt(A, S, "Druida")
        ...
        ObjData(Object).SkCarpinteria = INIDarClaveInt(A, S, "SkCarpinteria")
         ObjData(Object).SkAlquimia = INIDarClaveInt(A, S, "SkAlquimia")
         
   If ObjData(Object).SkAlquimia Then
        'ObjData(Object).Madera = val(GetVar(DatPath & "Obj.dat", "OBJ" & Object, "Madera"))
        ObjData(Object).Hierbas = INIDarClaveInt(A, S, "Hierbas")
    End If
        ...
        If ObjData(Object).ObjType = OBJTYPE_MERCENARIO Then
            ObjData(Object).InvocaN = INIDarClaveInt(A, S, "Invoca")
...
'A = INICarga(DatPath & "Mascotas.dat")
'Call INIConf(A, 0, "", 0)


'Dim loopMascotas As Integer
'For loopMascotas = 1 To NumMasctDatas
'X = INIBuscarSeccion(A, "M" & loopMascotas)
'If X >= 0 Then

'            ObjData(loopMascotas + NumObjDatas).Name = INIDarClaveStr(A, X, "Name")
'            ObjData(loopMascotas + NumObjDatas).MinHIT = val(INIDarClaveInt(A, X, "MinHit"))
'            ObjData(loopMascotas + NumObjDatas).MaxHIT = val(INIDarClaveInt(A, X, "MaxHit"))
'            ObjData(loopMascotas + NumObjDatas).ObjetoMascota = val(INIDarClaveInt(A, X, "ObjetoMascota"))
'            ObjData(loopMascotas + NumObjDatas).level = val(INIDarClaveInt(A, X, "Level"))
'            ObjData(loopMascotas + NumObjDatas).MinExp = val(INIDarClaveInt(A, X, "MinExp"))
'            ObjData(loopMascotas + NumObjDatas).MaxExp = val(INIDarClaveInt(A, X, "MaxExp"))
'            ObjData(loopMascotas + NumObjDatas).Alias = INIDarClaveStr(A, X, "Alias")
'            ObjData(loopMascotas + NumObjDatas).MaxDef = val(INIDarClaveInt(A, X, "MaxDef"))
'            ObjData(loopMascotas + NumObjDatas).MinDef = val(INIDarClaveInt(A, X, "MinDef"))
'            ObjData(loopMascotas + NumObjDatas).MinDefMag = val(INIDarClaveInt(A, X, "MinDefMag"))
'            ObjData(loopMascotas + NumObjDatas).MaxDefMag = val(INIDarClaveInt(A, X, "MaxDefMag"))
'            ObjData(loopMascotas + NumObjDatas).MinHITMag = val(INIDarClaveInt(A, X, "MinHITMag"))
'            ObjData(loopMascotas + NumObjDatas).MaxHITMag = val(INIDarClaveInt(A, X, "MaxHITMag"))
'            ObjData(loopMascotas + NumObjDatas).Ropaje = val(INIDarClaveInt(A, X, "NumRopaje"))
'            ObjData(loopMascotas + NumObjDatas).ObjType = OBJTYPE_MASCOTA
'            ObjData(loopMascotas + NumObjDatas).NoSeCae = INIDarClaveInt(A, X, "NoSeCae")
'End If
'DoEvents
'Next
        ...
        For x = XMinMapSize To XMaxMapSize
           ...
           ' Next
            ...
            If InfoTile And 32 Then
                ...
                'Debug.Print MapData(Map, x, Y).NpcIndex
...
'Npclist(NpcIndex).ReyC = val(GetVar(npcfile, "NPC" & NPCNumber, "ReyC"))
...
Call WriteVar(CharPath & "\" & UserList(BannedIndex).Name & ".chr", "FLAGS", "RAZON", Motivo)
...
Call WriteVar(CharPath & BannedIndex & ".chr", "FLAGS", "RAZON", Motivo)
