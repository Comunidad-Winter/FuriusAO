...
'FuriusAO Meditacion CAOS, REAL y Neutral 45
Public Const FXMEDITAREXTRAGRANDE = 25
Public Const FXMEDITARGRANDECAOS = 18
Public Const FXMEDITARGRANDEREAL = 19
'FuriusAO
Public Const POSINVALIDA = 3
Public Const POSINVALIDB = 11
...
Public Const NPCTYPE_DM = 9
Public Const NPCTYPE_CARCEL = 10
...
Public Const Botanica = 24
Public Const Alquimia = 25
...
Public Const OBJTYPE_MERCENARIO = 51
Public Const OBJTYPE_MASCOTA = 50
Public Const OBJTYPE_QUEST = 52
...
Public Const FONTTYPE_furius = "~255~150~50~1~0"
...
'Leito FONt Castillo = a Furius pero sin negrita :P
Public Const FONTTYPE_FUERTE = "~255~150~50~0~0"
'Leito.
'COLORES RMSG FuriusAO Staff
Public Const FONTTYPE_AZUL = "~0~0~255~0~0"
Public Const FONTTYPE_AMARILLO = "~255~255~0~0~0"
Public Const FONTTYPE_CELESTE = "~0~255~255~0~0"
Public Const FONTTYPE_BLANCO = "~255~255~255~0~0"
Public Const FONTTYPE_VERDE = "~0~255~0~0~0"
'COLORES RMSG FUriusAO Staff
...
Type tHechizo
    ...
    Target As Byte
'FIXIT: Declare 'End Type' con un tipo de datos de enlace en tiempo de compilación         FixIT90210ae-R1672-R1B8ZE
End Type
...
Type Inventario
    ...
    BarcoSlot As Byte
    NroItems As Integer
    MascotaEqpObjIndex As Integer
    MascotaEqpSlot As Byte
End Type
...
Public OfertasAct As Boolean

Public FPSBajos As Boolean

Public RetoEnCurso As Boolean
Public Reto2vs2EnCursO As Boolean
Public TiempoReto As Integer
Public RetoJ(2) As Integer
Public RetoParticipantes(2) As Integer
Public SoporteDesactivado As Boolean
Public ConNoche As Boolean
'RETO


Public Const MapaJuego As Byte = 190 ' aca va su mapa de juego


Type UserFlags
    Bovediando As Byte
    DevolvioProcesos As Byte
    EnDM As Boolean
    DmKills As Integer
    ...
    ConsejoCaoz As Byte
    ConsejoCiuda As Byte
    
    AyudanteCaoz As Byte
    AyudanteCiuda As Byte
    ...
    MatadasenR As Integer
    PerdidasenR As Integer
...
'FIXIT: En Visual Basic .NET no se admiten matrices con límite inferior distinto de cero     FixIT90210ae-R9815-H1984
Public Tops(1 To 3, 1 To 20) As Top
...
Public ObjBotanica() As Integer 'botanica leito
...
Public Ofertas As New cCola
Public PPP As New cCola
'Leito
Public Ayuda As New cCola
Public Consultas As New cCola
Public Consulta As String
Public Resnick As String
...
Public Sub HacerMancha(IndexManchaAnterior As Integer, IndexNuevaMancha As Integer)

UserList(IndexNuevaMancha).flags.Tocadas = UserList(IndexNuevaMancha).flags.Tocadas + 1 'pierde vida

If UserList(IndexNuevaMancha).flags.Tocadas >= 4 Then ' si perdio todas las vidas

    Call WarpUserChar(IndexNuevaMancha, 1, 50, 50) 'cambien esta posicion
    UserList(IndexNuevaMancha).flags.Mancha = False ' le saco la mancha
    UserList(IndexNuevaMancha).flags.Tocadas = 0 ' EDITO, AGREGO ESTO PARA QUE LE SAQUE LAS VIDAS CUANDO PIERDE.
    Call SendData(ToAll, 0, 0, "||" & UserList(IndexNuevaMancha).Name & " Perdió todas sus vidas, eligiendo mancha al azar." & FONTTYPE_furius)

    'Call ManchaAlAzar(IndexManchaAnterior) ' el index es, por si "tocó" al otro user y ganó

'Else

UserList(IndexManchaAnterior).flags.Mancha = False ' ya no es la mancha
UserList(IndexNuevaMancha).flags.Mancha = True 'convierto al nuevo user en la mancha
Call SendData(ToAll, 0, 0, "||Ahora " & UserList(IndexNuevaMancha).Name & " es la mancha! corran!" & FONTTYPE_furius)

Call SendData(ToPCArea, IndexNuevaMancha, UserList(IndexNuevaMancha).POS.Map, "CFX" & UserList(IndexNuevaMancha).Char.CharIndex & "," & 32 & "," & LoopAdEternum)

End If

End Sub

Public Sub ManchaAlAzar()

'Dim HayMancha As Boolean

'BuclePpal:

'HayMancha = False

'For i = 1 To userson 'hago un bucle con todos los usuarios
'If UserList(i).POS.Map = 190 And Not UserList(userindex).flags.Privilegios > 0 Then 'si ta jugando y NO ES GM
'   If RandomNumber(1, 3) = 2 Then 'tiro un random a ver si es o no mancha
'        HayMancha = True 'IMPORTANTISIMO
'        UserList(i).flags.Mancha = True 'convierto al nuevo user en la mancha''
'
'         Call SendData(ToMap, i, 0, "||Ahora " & UserList(i).Name & " es la mancha! corran!" & FONTTYPE_INFO)
'        Call SendData(ToPCArea, i, UserList(i).POS.Map, "CFX" & UserList(i).Char.CharIndex & "," & 32 & "," & LoopAdEternum)
'       Exit For
'    End If
'End If
'Next i

'If Not HayMancha Then GoTo BuclePpal 'SIEMPRE vamos a encontrar una mancha

End Sub

'Public Sub DarRecompenza(userindex As Integer)

'Call MeterItemEnInventario(userindex, NRO, Cantidad)
'UserList(userindex).Stats.GLD = UserList(userindex).Stats.GLD + X
'UserList(userindex).Stats.Exp = UserList(userindex).Stats.Exp + X
