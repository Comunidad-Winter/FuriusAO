...
Private lFrameLimiter As Currency
...
Public AoS As New PackerFuriusAO.GamePaks
...
Recompensas(MAGO, 1, 2).Descripcion = "Petrifica por un momento al npc usando 250 de mana."
...
Recompensas(NIGROMANTE, 1, 2).Name = "Petrificar"
Recompensas(NIGROMANTE, 1, 2).Descripcion = "Petrifica por un momento al npc usando 250 de mana."
...
Recompensas(PALADIN, 1, 2).Name = "Petrificar"
Recompensas(PALADIN, 1, 2).Descripcion = "Petrifica por un momento al npc usando 250 de mana."
...
Recompensas(CLERIGO, 1, 2).Name = "Petrificar"
Recompensas(CLERIGO, 1, 2).Descripcion = "Petrifica por un momento al npc usando 250 de mana."
...
Recompensas(BARDO, 1, 2).Name = "Petrificar"
Recompensas(BARDO, 1, 2).Descripcion = "Petrifica por un momento al npc usando 250 de mana."
...
Recompensas(BARDO, 3, 1).Name = "Resistencia Mágica"
Recompensas(BARDO, 3, 1).Descripcion = "Cuando tengas equipado el Laúd tendrás 5% de resistencia mágica."

Recompensas(BARDO, 3, 2).Name = "Melodía Caótica"
Recompensas(BARDO, 3, 2).Descripcion = "Aumenta el daño base del Apocalipsis y la Descarga Electrica en 5 puntos cuando tengas equipado el laúd."
...
Recompensas(DRUIDA, 1, 2).Name = "Petrificar"
Recompensas(DRUIDA, 1, 2).Descripcion = "Petrifica por un momento al npc usando 250 de mana."

Recompensas(DRUIDA, 2, 1).Name = "Don de magia."
Recompensas(DRUIDA, 2, 1).Descripcion = "Permite a nuestro héroe equipar ítems de tipo ''Hechizero''"

Recompensas(DRUIDA, 2, 2).Name = "Fortaleza de Guerrero"
Recompensas(DRUIDA, 2, 2).Descripcion = "Le posibilita a nuestro personaje el uso de nuevas armas, espadas, cascos, escudos y armaduras."

Recompensas(DRUIDA, 3, 1).Name = "Legado divino"
Recompensas(DRUIDA, 3, 1).Descripcion = "Concede a nuestro héroe, el poder de invocar elementales de tierra y fuego para que luchen a tu lado."

Recompensas(DRUIDA, 3, 2).Name = "Domador experto"
Recompensas(DRUIDA, 3, 2).Descripcion = "Permite a nuestro héroe el poder de domar como maximo dos criaturas de las tierras de FúriusAO para que luchen a tu lado."
...
Recompensas(ASESINO, 1, 2).Name = "Petrificar"
Recompensas(ASESINO, 1, 2).Descripcion = "Petrifica por un momento al npc usando 250 de mana."
...
Recompensas(CAZADOR, 1, 2).Name = "Petrificar"
Recompensas(CAZADOR, 1, 2).Descripcion = "Petrifica por un momento al npc usando 250 de mana."
...
Recompensas(PIRATA, 3, 1).Name = "Reducción de Parálisis"
Recompensas(PIRATA, 3, 1).Descripcion = "La parálisis lanzada hacia el pirata dura 5 segundos."
...
AuxInteger = SD(N)
AuxInteger2 = SDM(N)
ValidarLoginMSG = Complex(AuxInteger + AuxInteger2 + 10)
End Function
...
If UCase$(Left$(Datos, 3)) = "/GM" Then
  '  frmMSG.Show
   Call AddtoRichTextBox(frmMain.RecTxt, "¡Para mandar un soporte, debera utilizar nuestro sistema web, www.furiusao.com.ar Muchas Gracias...", 255, 0, 0, True, False, False)
    Exit Sub
End If
...
If UserMoving = 0 Then
    If Not UserEstupido Then
        ...
        If GetKeyState(CustomKeys.BindedKey(eKeyType.mKeyRight)) < 0 And GetKeyState(vbKeyShift) >= 0 Then
            Call MoveMe(EAST)
            Exit Sub
        End If
        ...
        If GetKeyState(CustomKeys.BindedKey(eKeyType.mKeyLeft)) < 0 And GetKeyState(vbKeyShift) >= 0 Then
              Call MoveMe(WEST)
              Exit Sub
        End If
    Else
...
End Sub
Sub LoadMMapx(Mmap As Integer)
'Modificado por Ruffneck.~ y ^[GS]^

frmMmap.PicMMAp.Picture = LoadPicture(App.Path & "\minimap\" & Val(Mmap) & ".bmp")
...
AddtoRichTextBox frmCargando.Status, "Comprobando actualizaciones...", 255, 150, 50, 1, , False

Dim A1, A2 As String
A1 = Val(FrmIntro.Actualizo.OpenURL("http://ao.localstrike.com.ar/au/Num.ini"))
Dim fnum As Integer
fnum = FreeFile
Open App.Path & "\INIT\Actualizaciones.dat" For Input As fnum
    Do While Not EOF(fnum)
        Line Input #fnum, A2
    Loop
    Close fnum
If Val(A1) > Val(A2) Then
If MsgBox("Se han encontrado actualizaciones pendientes!!!, desea descargarlas ahora?", vbYesNo) = vbYes Then
Shell App.Path & "\AutoUpdate FuriusAO.exe"
End
End If
End If
...
AddtoRichTextBox frmCargando.Status, "Cargados con éxito", 255, 150, 50, 1, , False
...
'FPS LIBRES
'Dim Pp As New Mi_timer ' es para el control de Fps
...
frmPres.Picture = LoadPicture(App.Path & "\Graficos\ls.jpg")
...
'FPS
'Pp.init 1000, 56 ' le doy el tiempo
'Set MotorG.MiBltSup = BackBufferSurface
'MotorG.MiMRect = MainDestRect
'MotorG.MiPMRect = MainViewRect
'Set MotorG.MiSupPrincipal = PrimarySurface
'MotorG.BPP = 16
...
'Call SendData("COMUSUOK")
          ...
          Call RenderScreen(UserPos.X - AddtoUserPos.X, UserPos.Y - AddtoUserPos.Y, OffsetCounterX, OffsetCounterY)
           ...
           If BancoT > 0 Then Call Dialogos.DrawText(260, 260, "MODO BANCO", vbRed)

          If ModoTrabajo Then Call Dialogos.DrawText(260, 260, "MODO TRABAJO", vbRed)
           ' If FP Then
               If frmMain.Pocho = False Then
                    Call Dialogos.DrawText(150, 700, frmMain.mapa.Caption, vbCyan)
                    Call Dialogos.DrawText(150, 690, "FPS: " & frmMain.fpstext.Caption, vbCyan)
                    Call Dialogos.DrawText(150, 680, "Online: " & frmMain.NumOnline.Caption, vbCyan)
                End If
             
                If frmOpciones.Clanesx = 1 Then
                Dim Fer2 As Integer
             
                    For Fer2 = 1 To 7
                        If MensajesP(Fer2) <> "" Then
                             Call Dialogos.DrawText(260, 665 - (Fer2 * 12), MensajesP(Nmensajes - Fer2 + 1), vbCyan)
                        End If '(Fer2 * 12)
                    DoEvents
                    Next Fer2
                End If
 ...
 '       Debug.Print (DirectX.TickCount - lFrameTimer) / 60
        While (DirectX.TickCount - lFrameTimer) / 60 < FramesPerSecCounter
        Sleep 2
        Wend
        
        If DirectX.TickCount - lFrameTimer > 1000 Then
            FramesPerSec = FramesPerSecCounter
            If FPSFLAG Then frmMain.Caption = "Furius AO" & " V " & App.Major & "." & App.Minor & "." & App.Revision
