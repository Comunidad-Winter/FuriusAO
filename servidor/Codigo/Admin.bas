...
Else
   ...
   ' Call SendData(ToIndex, userindex, 0, "||" & "Has sido encarcelado por " & Arg1 & " durante los próximos " & Minutos & " minutos." & " GM: " & GmName & FONTTYPE_BLANCO)
   ' Call SendData(ToIndex, userindex, 0, "30")
End If
...
tBan.FechaLiberacion = (Ahora + Dias)
tBan.Causa = Causa
tBan.Baneador = UCase$(Baneador)

Call Baneos.Add(tBan)
Call SaveBan(Baneos.Count)
Call SendData(ToAdmins, 0, 0, "||" & Nombre & " fue baneado por " & Causa & " durante los próximos " & Dias & " días." & FONTTYPE_furius)
